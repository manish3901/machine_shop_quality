...
def production_entry_form():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            planned_qty = int(request.form['planned_quantity'])
            actual_qty = int(request.form['actual_quantity']) if request.form.get('actual_quantity') else 0
            ...
            prod_entry = ProductionEntry(
                ...
                cutlength=float(request.form['cutlength']) if request.form.get('cutlength') else None,
                ...
                ideal_cycle_time=float(request.form['ideal_cycle_time']) if request.form.get('ideal_cycle_time') and not request.form.get('no_cycle_time') else None,
                total_time_taken_minutes=int(total_runtime_mins),
                total_ideal_time_minutes=total_ideal_mins,
                downtime_minutes=int(request.form.get('total_planned_downtime', 0)),
            ...
            for i in range(len(dt_reason_ids)):
                if dt_reason_ids[i] and dt_durations[i]:
                    dt_record = ProductionPlannedDowntime(
                        production_entry_id=prod_entry.id,
                        reason_id=int(dt_reason_ids[i]),
                        duration_minutes=int(dt_durations[i])
                    )
                    db.session.add(dt_record)
...
def edit_entry(entry_id):
    ...
    if request.method == 'GET':
        ...
        reasons = DowntimeReason.query.filter_by(status='Active', is_fixed=False).all()
        fixed_reasons = DowntimeReason.query.filter_by(status='Active', is_fixed=True).all()
        fixed_downtime_total = sum(r.default_minutes for r in fixed_reasons if r.default_minutes)
        ...
        try:
            ...
            entry.customer_id = int(request.form['customer_id'])
            entry.operation_type_id = int(request.form.getlist('operation_type_id[]')[0]) if request.form.getlist('operation_type_id[]') else None
            entry.section_number = request.form.get('section_number')
            entry.cutlength = float(request.form['cutlength']) if request.form.get('cutlength') else None
            entry.planned_quantity = int(request.form['planned_quantity'])
            entry.actual_quantity = int(request.form['actual_quantity']) if request.form.get('actual_quantity') else 0
            self_rejection_rows, self_rejection_qty, self_rejection_weight_per_pcs, machining_scrap_weight_kg = _parse_self_rejection_rows(request.form)
            ...
            entry.ideal_cycle_time = float(request.form['ideal_cycle_time']) if request.form.get('ideal_cycle_time') else 0.0
            entry.total_time_taken_minutes = int(total_runtime_mins)
            entry.total_ideal_time_minutes = total_ideal_mins
            entry.downtime_minutes = int(request.form.get('total_planned_downtime', 0))
...
def login():
    ...
    try:
        ...
        user_query = text("""
            SELECT u.user_id, e.emp_name, u.email_login, u.role_id, u.is_active, u.password_hash
            FROM user_login u
